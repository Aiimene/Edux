  ...
  export default function EditStudentModal({ isOpen, onClose, studentId, studentData, onSave, onDelete }: any) {
    ...
    const mergedInitial = useMemo(() => {
      ...
      return {
        ...
        levels: Array.isArray(merged.levels) ? merged.levels.map((l: any) => l?.name || l).filter(Boolean) : [],
        ...
        feePayment: (merged.feePayment ?? merged.total_owed ?? merged.fee_payment ?? merged.fees_payment ?? merged.paid_fees ?? merged.payment) !== undefined ? String(merged.feePayment ?? merged.total_owed ?? merged.fee_payment ?? merged.fees_payment ?? merged.paid_fees ?? merged.payment) : String(placeholder.feePayment),
        enrollmentDate: merged.enrollmentDate ?? merged.enrollment_date ?? merged.date_joined ?? '',
        paymentMethod: (merged.paymentMethod || merged.payment_method) ? String(merged.paymentMethod || merged.payment_method).toLowerCase() : '',
        paymentStatus: (merged.paymentStatus || merged.payment_status) ? String(merged.paymentStatus || merged.payment_status).toLowerCase() : '',
